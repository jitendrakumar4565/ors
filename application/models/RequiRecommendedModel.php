...
class RequiRecommendedModel extends CI_Model {
    ...
    private function listquery($cond) {
        $this->db->select("req.mst_id,req.auto_id,req.c_mode_recruitment,org.org_name,dept.dept_name,req.officer_name,desig.desig_name,pp.post_name,req.entry_datetime,req.advertised_date,req.recommended_datetime,req.seen_flag,req.action_taken,req.action_datetime,req.seen_by_dept,req.d_ur,req.d_apst,req.rec_ur,req.rec_apst");
    ...
    /*
      function updatePostRecommendedModel($cond, $data) {
      $this->db->where($cond);
      $res = $this->db->update('recomm_mapped_requi_inbox', $data);
      if ($res) {
      return TRUE;
      } else {
      return FALSE;
      ...
      $this->db->where($cond);
      return $this->db->get('recomm_mapped_requi_inbox')->result();
      }

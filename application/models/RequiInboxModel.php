...
class RequiInboxModel extends CI_Model {
    ...
    function selectDataForAddReccRowModel($select, $cond) {
        $this->db->select($select);
        $this->db->where($cond);
        $this->db->join('mst_post pp', 'pp.auto_id=req.post_name', 'left');
        $this->db->join('mst_organisation org', 'org.auto_id=req.org_name', 'left');
    ...
    function selectDetailsRowModel($select, $cond) {
        $this->db->select($select);
        $this->db->where($cond);
        return $this->db->get('inbox_requisition')->row();
    }

    function updateDetailsModel($cond, $data) {
        $this->db->where($cond);
        $res = $this->db->update('inbox_requisition', $data);
        if ($res) {
            return TRUE;
        } else {
            return FALSE;
    ...
    function updateAcceptedListModel($cond) {
        $select = $this->db->select('*')->where($cond)->get('inbox_requisition');
        if ($select->num_rows()) {
            $this->db->insert_batch('accepted_requisition', $select->result_array());
        }
    }

    function updateReturnedListModel($cond) {
        $select = $this->db->select('*')->where($cond)->get('inbox_requisition');
        if ($select->num_rows()) {
            $this->db->insert_batch('returned_requisition', $select->result_array());
        }
    }

    function pullBackFormModel($cond, $data, $actionFlag) {
        if ($actionFlag == "ACCEPTED") {
            //$this->db->where($cond);
            //$this->db->delete('accepted_requisition');
            $this->db->where($cond);
            return $this->db->update('inbox_requisition', $data);
        } else if ($actionFlag == "RETURNED") {
            //$this->db->where($cond);
            //$this->db->delete('returned_requisition');
            $this->db->where($cond);
            return $this->db->update('inbox_requisition', $data);
        } else {
            return FALSE;

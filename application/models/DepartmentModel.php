...
class DepartmentModel extends CI_Model {
    ...
    var $column_order = array(null, 'dept_name'); //set column field database for datatable orderable
    var $column_search = array('dept_name'); //set column field database for datatable searchable 
    var $order = array('dept_name' => 'asc'); // default order 

    private function listquery($cond) {
        $this->db->select("auto_id,dept_name,dept_address");
    ...
    function selectDetailsModel($select, $cond) {
        ...
        $this->db->where($cond);
        return $this->db->get('mst_department')->result();
    }

    function selectDetailsRowModel($select, $cond) {
        $this->db->select($select);
        $this->db->where($cond);
        return $this->db->get('mst_department')->row();
    }

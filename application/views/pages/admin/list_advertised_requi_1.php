...
<div class="content-wrapper">
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>Advertised Requisitions</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="<?php echo base_url() ?>">Home</a></li>
                        <li class="breadcrumb-item active">Advertised Requisitions</li>
                    </ol>
                </div>
            </div>
        </div>
    ...
    <section class="content">
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="modal fade" id="modal_recommended">
                    <div class="modal-dialog modal-xl">
                        <div class="modal-body">
                            <div class="modal-content card card-primary card-outline">
                                <div class="card-header">                       
                                    ...
                                    <div class="float-right">
                                        <a href="javascript:closeModal('modal_recommended')" class="close btn btn-sm" title="Close">
                                            <span aria-hidden="true">X Close</span>
                                        </a>
                                    </div>                                        
                                </div>
                                <!-- /.card-header -->
                                <div class="card-body" id="draftRecommendedHTML">
                                </div>
                                <!-- /.card-body -->
                            </div>
                        </div>
...
<script>
    ...
    function reload_datatables() {
        myTable = $('#data_details_table').DataTable({
            ...
            "autoWidth": true,
            //dom: 'lBfrtip',
            ajax: {url: "<?php echo site_url('requiAdvertised/ajax_list') ?>",
                type: "POST",
                data: function (data) {
    ...
    function addDraftRecommended(id) {
        $('#loading').show();
        jQuery.ajax({
            type: "POST",
            url: "<?php echo base_url('recommendedDraft/addDraftRecommendedForm'); ?>",
            dataType: 'json',
            data: {
                id: id
            },
            success: function (data) {
                if (data.status) {
                    $('#draftRecommendedHTML').html(data.draftRecommendedHTML);
                    $('#modal_recommended').modal({backdrop: 'static', keyboard: false});
                    //$('.select2').select2();
                } else {
                    toastr.error(data.msg, "", {closeButton: true, timeOut: 5000});
                    if (data.logout) {

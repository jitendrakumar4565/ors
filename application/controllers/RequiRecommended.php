...
class RequiRecommended extends CI_Controller {

    function __construct() {
        parent::__construct();
        $this->load->model('RequiRecommendedModel', 'requiRecommended', TRUE);
    ...
    function index() {
        ...
        if ($userid != "" && in_array($role, array('SA', 'AD'))) {
            ...
            $this->load->view('pages/' . $res . '/list_recommended_requi');
    ...
    function ajax_list() {
        ...
        if ($userid != "" && in_array($role, array('SA', 'AD'))) {
            if (strtoupper($_SERVER['REQUEST_METHOD']) == 'POST') {
                $list = $this->requiRecommended->list_databales(array('req.action_taken' => 'RECOMMENDED', 'rec_sent_to_dept_flag' => 'F'));
                $count_rec = $this->requiRecommended->list_filtered(array('req.action_taken' => 'RECOMMENDED', 'rec_sent_to_dept_flag' => 'F'));
        ...
        if ($userid != "" && in_array($role, array('SA', 'AD'))) {
            if (strtoupper($_SERVER['REQUEST_METHOD']) == 'POST') {
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    $mData = $this->requiInbox->selectDetailsModel('auto_id', array('auto_id' => $auto_id, 'action_taken' => 'ADVERTISED'));
                    $this->db->trans_start();
                    if ($mData != NULL) {

...
class RequiAccepted extends CI_Controller {

    function __construct() {
        parent::__construct();
        $this->load->model('RequiAcceptedModel', 'requiAccepted', TRUE);
    ...
    function index() {
        ...
        if ($userid != "" && in_array($role, array('SA', 'AD'))) {
            ...
            $this->load->view('pages/' . $res . '/list_accepted_requi');
    ...
    function ajax_list() {
        ...
        if ($userid != "" && in_array($role, array('SA', 'AD'))) {
            if (strtoupper($_SERVER['REQUEST_METHOD']) == 'POST') {
                $list = $this->requiAccepted->list_databales(array('req.action_taken' => 'ACCEPTED'));
                $count_rec = $this->requiAccepted->list_filtered(array('req.action_taken' => 'ACCEPTED'));
                $data = array();
                $no = $_POST['start'];
                if ($list != NULL) {
                    foreach ($list as $accrow) {
                        ...
                        $pullBack = "";
                        if ($accrow->seen_by_dept == 0 && in_array($actionTaken, array('ACCEPTED'))) {
                        ...
                        $isAdvertised = "";
                        if (in_array($actionTaken, array('ACCEPTED'))) {
                            $isAdvertised = '<a title="Advertised" class="btn btn-outline-primary btn-xs" href="javascript:void(0)" onclick=advertisedPost("' . $accrow->auto_id . '")><i class="fa fa-newspaper-o"></i> Advertise</a>&nbsp;';
                        }
                        ...
                        $row[] = $preview . $previewPdf . $pullBack . $isAdvertised;
                        $data[] = $row;
                    }
